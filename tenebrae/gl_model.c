...
void Mod_LoadFaces (lump_t *l)
{
	...
	for ( surfnum=0 ; surfnum<count ; surfnum++, in++, out++)
	{
		...
		//Con_Printf("edges %d",out->numedges);
		out->flags = 0;
...
/*#ifndef QUAKE2
		...
		if (!Q_strncmp(out->texinfo->texture->name,"*",1))		// turbulent
		{
			...
			if (strstr (out->texinfo->texture->name, "water")) {
				if (strstr (out->texinfo->texture->name, "mirror") || mir_forcewater.value)
					out->flags |= SURF_MIRROR;
			}


			if (strstr (out->texinfo->texture->name, "glass")) {
		...
		//XYZ
		if (strstr (out->texinfo->texture->name, "caulk"))
		{
...
void Mod_LoadLeafs (lump_t *l)
{
	...
	for ( i=0 ; i<count ; i++, in++, out++)
	{
		...
		{
		int newnum = out->nummarksurfaces;
		...
		for (j=0; j<out->nummarksurfaces; j++) {
			if (out->firstmarksurface[j] != NULL) {
				out->firstmarksurface[newnum] = out->firstmarksurface[j];
				newnum++;
			}
		}
		out->nummarksurfaces = newnum;
