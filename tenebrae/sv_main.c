...
// send an update
	...
	//
		...
		//PENTA: new baseline fields (Fixed!)
		if (bits & U_COLOR) {
			for (i=0 ; i<3 ; i++)
			{
...
void SV_CreateBaseline (void)
{
	...
	//-=-WARNING-=-WARNING-=-WARNING-=-WARNING-=-WARNING-=-WARNING-=-WARNING-=-WARNING
	//-=-WARNING-=-WARNING-=-WARNING-=-WARNING-=-WARNING-=-WARNING-=-WARNING-=-WARNING
	// this code is duplicated elsewhere on the server side
	// but on the client side the same parsing routine is used
	...
	//
		...
		//PENTA: new baseline fields
		MSG_WriteByte (&sv.signon, (int)(svent->baseline.alpha*255));
		MSG_WriteByte (&sv.signon, svent->baseline.style);
		MSG_WriteShort (&sv.signon, svent->baseline.light_lev);
		MSG_WriteByte (&sv.signon,  svent->baseline.pflags);
		for (i=0 ; i<3 ; i++)
		{
			MSG_WriteCoord(&sv.signon, svent->baseline.origin[i]);
			MSG_WriteAngle(&sv.signon, svent->baseline.angles[i]);
			MSG_WriteByte(&sv.signon, (int)(svent->baseline.color[i]*255));
		}
	}
}
