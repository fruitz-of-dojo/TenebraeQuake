...
entity_t		*cl_lightvisedicts[MAX_VISEDICTS];
...
// rotate binary objects locally
		...
		//PENTA: After tutorial done by james.dale@ukonline.co.uk
		if (ent->effects & EF_BLUE)
		{
			dl = CL_AllocDlight (i);
			VectorCopy (ent->origin,  dl->origin);
			dl->origin[2]+=16; //dont let light be to close on ground
			dl->radius = 200 + (rand()&31);
			dl->die = cl.time + 0.001;
			dl->color[0] = 0.5;
			dl->color[1] = 0.5;
		...
		if (ent->effects & EF_FULLDYNAMIC)
		{
			...
			VectorCopy(ent->angles,dl->angles);

			dl->pflags = ent->pflags;
		}
		...
		else if (ent->model->flags & EF_ROCKET)
		{
			...
			//PENTA: let a grenade give off light
			dl = CL_AllocDlight (i);
			VectorCopy (ent->origin, dl->origin);
			dl->radius = 200;
			dl->die = cl.time + 0.01;
